...
.lang-btn, .theme-btn {
  ...
  padding: 5px 10px; border-radius: 20px; cursor: pointer; font-size: .8rem;
  transition: background .2s, color .2s;
}
.lang-btn:hover, .theme-btn:hover { background: var(--gold); color: #1a1305; }
...
input[type=date], input[type=file], select {
  width: 100%; padding: 11px 13px; border-radius: 10px; border: 1px solid var(--input-border);
  background: var(--input-bg); color: var(--text); font-size: .95rem; font-family: inherit;
}
...
.glow-form {
  box-shadow: 0 0 20px var(--glow-color), 0 0 60px rgba(212,175,55,0.15);
  border-color: var(--glow-color);
  transition: box-shadow 0.3s ease, border-color 0.3s ease;
}
.glow-form:hover {
  box-shadow: 0 0 30px var(--glow-color), 0 0 80px rgba(212,175,55,0.2);
}
...
body.theme-dark {
  ...
  --input-bg: #0a1a2b;
  --input-border: #2a5a7a;
  --shadow: 0 10px 30px rgba(0,40,70,.5);
  --glow-color: #6bb8e0;
  background:
    radial-gradient(circle at 15% 15%, rgba(107,184,224,.12), transparent 40%),
    radial-gradient(circle at 85% 10%, rgba(107,184,224,.08), transparent 35%),
    linear-gradient(180deg, #0a1a2b 0%, #0d2840 60%, #123a55 100%);
}
body.theme-dark .btn {
  background: linear-gradient(135deg, #4a9fc9, #2a7aa8);
  color: #fff;
}
...
body.theme-dark .navbar { background: rgba(10,26,43,.85); }
body.theme-dark .brand { color: #8fd4f0; }
body.theme-dark .brand span { color: #7ab3d0; }
body.theme-dark .badge { background: rgba(107,184,224,.2); color: #8fd4f0; }
body.theme-dark .stars { color: #8fd4f0; }
body.theme-dark .price { color: #7dd4a0; }
body.theme-dark .flight-row, .theme-dark .class-opt { background: #0a1a2b; }
body.theme-dark .tab { background: #0a1a2b; }
body.theme-dark .airline-card, .theme-dark .hotel-card { background: #0a2238cc; }
body.theme-dark h1, .theme-dark h2, .theme-dark h3 { color: #8fd4f0; }
body.theme-dark .flash.ok { background: #0a2a3a; color: #8fd4f0; border-color: #4a9fc9; }
body.theme-dark .flash.error { background: #2a1a1a; color: #f3a49c; border-color: #6b2a24; }
body.theme-dark .nav-links a:hover { color: #8fd4f0; }
body.theme-dark .lang-btn, .theme-dark .theme-btn { border-color: #2a5a7a; }
body.theme-dark .lang-btn:hover, .theme-dark .theme-btn:hover { background: #4a9fc9; color: #fff; }
body.theme-dark .glow-form {
  box-shadow: 0 0 20px var(--glow-color), 0 0 60px rgba(107,184,224,0.15);
}
body.theme-dark .glow-form:hover {
  box-shadow: 0 0 30px var(--glow-color), 0 0 80px rgba(107,184,224,0.2);
}

/* English Direction */
body.lang-en { direction: ltr; text-align: left; }
body.lang-en .navbar { direction: ltr; }
body.lang-en .wrap { direction: ltr; text-align: left; }
body.lang-en table th, body.lang-en table td { text-align: left; }
body.lang-en .flight-row { direction: ltr; }
body.lang-en .search-bar { direction: ltr; }
"""
...
def layout(title_key, body, user=None):
    """Generate page layout"""
    from translations import t, get_language, get_theme
    
    lang = get_language()
    theme = get_theme()
    
    title = t(title_key, lang) if title_key in ['page_login', 'page_signup', 'page_home', 'page_airline', 'page_select_class', 'page_hotel_rooms', 'page_book_room', 'page_booking_detail', 'page_my_bookings'] else title_key
    
    lang_toggle = f'''
        <a href="{url_for('set_lang', lang='ar')}" class="lang-btn" title="{t('arabic', lang)}">🇸🇦 عربي</a>
        <a href="{url_for('set_lang', lang='en')}" class="lang-btn" title="{t('english', lang)}">🇬🇧 English</a>
    '''
    
    theme_toggle = f'''
        <a href="{url_for('set_theme_route', theme='light')}" class="theme-btn" title="{t('theme_light', lang)}">☀️</a>
        <a href="{url_for('set_theme_route', theme='dark')}" class="theme-btn" title="{t('theme_dark', lang)}">🌙</a>
    '''
    
    nav_links = ""
    if user:
        nav_links = f'''
            <a href="{url_for('my_bookings')}">{t('bookings', lang)}</a>
            <a href="{url_for('home')}">{t('airlines', lang)}</a>
            <span style="color:var(--sand)">{user['username']}</span>
            <a href="{url_for('logout')}" class="btn small danger">{t('logout', lang)}</a>
        '''
    else:
        nav_links = f'<a href="{url_for("login")}" class="btn small">{t("login", lang)}</a>'
    
    body_classes = f"theme-{theme} lang-{lang}"
...
<title>{title} - {t('welcome', lang)}</title>
...
<div class="navbar">
  <div class="brand">{t('brand', lang)} <span>{t('sub_brand', lang)}</span></div>
  <div class="nav-links">
    {nav_links}
...
<div class="footer-note">{t('footer', lang)}</div>
</body>
</html>"""
